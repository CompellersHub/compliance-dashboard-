...
import { AlertTriangle, CheckCircle, FileText, Globe, User } from "lucide-react";
...
export function ScreeningResults() {
  ...
  if (isLoading) return <div className="p-4 text-center">Loading screening results...</div>;
  if (error) return <div className="p-4 text-red-500">Error: {error}</div>;
  if (!results) return <div className="p-4 text-gray-500">No screening results available. Perform a search first.</div>;

  // The API response is now in results.data array
  const screeningData = results.data?.[0] || results;
      ...
      {screeningData.matches?.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <p className="text-xs text-blue-700 dark:text-blue-400 mt-1">
            Found {screeningData.matches.length} potential matches across watchlists
          </p>
        </div>
      )}
      ...
      <Card className="overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-bold">{screeningData.fullName}</CardTitle>
            <Badge
              variant="outline"
              className={`${
                screeningData.riskLevel === "HIGH"
              ...
              {screeningData.matches.map((match, index) => (
                <div key={index} className="mb-4 last:mb-0">
                  ...
                  <div className="text-sm space-y-2">
                    ...
                    <p><strong>List Type:</strong> {match.details?.listType}</p>
                    <p><strong>Committee:</strong> {match.details?.committee}</p>
                    <p><strong>Last Updated:</strong> {new Date(match.details?.lastUpdated).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Tabs defaultValue="analysis" className="p-4">
            ...
            <TabsContent value="analysis" className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm">{screeningData.aiAnalysis?.detailedAnalysis}</p>
              </div>
              <div>
                <h4 className="font-medium mb-2">Risk Factors</h4>
                <ul className="list-disc pl-5 space-y-1 text-sm">
                  {screeningData.aiAnalysis?.riskFactors?.map((factor, index) => (
              ...
              <div>
                ...
                <ul className="list-disc pl-5 space-y-1 text-sm">
                  {screeningData.aiAnalysis?.recommendations?.map((rec, index) => (
                    <li key={index}>{rec}</li>
                  ))}
                </ul>
              </div>
            ...
            <TabsContent value="news">
              <div className="space-y-4">
                <h4 className="font-medium">News Mentions ({screeningData.newsArticles?.length || 0})</h4>
                {screeningData.newsArticles?.length > 0 ? (
                  screeningData.newsArticles.map((article, index) => (
                    <div key={index} className="text-sm border-b pb-4 last:border-b-0 last:pb-0">
                      <a 
                        href={article.url} 
                        target="_blank" 
            ...
            <TabsContent value="bio" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">{screeningData.aiAnalysis?.bioDetails?.dateOfBirth || "Not available"}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Place of Birth</p>
                  <p className="text-sm">{screeningData.aiAnalysis?.bioDetails?.placeOfBirth || "Not available"}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Nationality</p>
                  <p className="text-sm">{screeningData.aiAnalysis?.bioDetails?.nationality || "Not available"}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Occupation</p>
                  <p className="text-sm">{screeningData.aiAnalysis?.bioDetails?.occupation || "Not available"}</p>
                </div>
                <div className="md:col-span-2">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Education</p>
                  <p className="text-sm">{screeningData.aiAnalysis?.bioDetails?.education || "Not available"}</p>
                </div>
              </div>
              {screeningData.wikipediaData && (
                <div className="mt-4">
                  ...
                  <p className="text-sm mt-1">{screeningData.wikipediaData.extract}</p>
                </div>
              )}
            </TabsContent>
          </Tabs>
